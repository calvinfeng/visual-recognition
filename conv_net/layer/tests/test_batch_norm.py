...
class BatchNormLayerTest(unittest.TestCase):
    ...
    def test_forward_pass(self):
        ...
        for t in range(50):
            ...
            self.layer.forward_pass(acts, gamma, beta, bn_param)

        x = np.random.randn(N, D1)
        acts = np.maximum(0, x.dot(W1)).dot(W2)
        norm_x = self.layer.forward_pass(acts, gamma, beta, bn_param)
    ...
    def test_backward_pass(self):
        ...
        bn_param = {'mode': 'train'}
        fx = lambda x: self.layer.forward_pass(x, gamma, beta, bn_param)
        fg = lambda g: self.layer.forward_pass(x, g, beta, bn_param)
        fb = lambda b: self.layer.forward_pass(x, gamma, b, bn_param)
        ...
        self.layer.forward_pass(x, gamma, beta, bn_param)
